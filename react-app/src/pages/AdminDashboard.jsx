...
import {
  ...
  getEvents, addEvent, deleteEvent, updateEvent, uploadEventImage,
...
export default function AdminDashboard() {
  ...
  const [newEvent,       setNewEvent]       = useState({ title:'', date:'', time:'', registration_deadline:'', location:'', description:'', fee:'', payment_account:'', audience:'both', banner_image_file: null })
  ...
  const handleAddEvent = async (e) => {
    ...
    if (ok) {
      ...
      setEvents(prev => [{ ...createdEvent, banner_image_file: null }, ...prev])
      setStats(s => ({ ...s, events: s.events + 1 }))
      setNewEvent({ title:'', date:'', time:'', registration_deadline:'', location:'', description:'', fee:'', payment_account:'', audience:'both', banner_image_file: null })
      setShowModal(false)
    } else {
  ...
  const handleUpdateEvent = async (e) => {
    ...
    if (ok) {
      ...
      setEvents(prev => prev.map(ev => ev.id === editingEvent.id ? { ...editingEvent, fee: Number(editingEvent.fee)||0, banner_image: bannerImage, banner_image_url: bannerImageUrl, banner_image_file: null } : ev))
